...
function renderDieren(){
  ...
  h+=`<div class="dkop"><h2>Alle dieren</h2></div>`+
    ...
    `<div class="dchips">`+
      `<button type="button" class="dchip dfilter${window._dgespot?' on':''}" id="dfilter">Eerder gespot<span>${DIER_LIJST.filter(d=>tel[d.k]).length}</span></button>`+
      DIER_GROEP.map(([g,label])=>{ const ds=perGroep(g), gs=ds.filter(d=>tel[d.k]).length;
      return `<button type="button" class="dchip" data-groep="${g}" style="${kleurVan(g)}">${esc(label)}<span>${gs}/${ds.length}</span></button>`; }).join('')+`</div>`+
    `<div id="dalle">`+DIER_GROEP.map(([g,label])=>{ const ds=perGroep(g); if(!ds.length) return ''; const gs=ds.filter(d=>tel[d.k]).length;
      return `<section class="dgroep" id="dg-${g}" style="${kleurVan(g)}"><div class="dkop"><h2>${esc(label)}</h2><span class="dsub">${gs} van ${ds.length}</span></div><div class="dlist">`+
...
const APP_VERSIE='2026-09-10-139';
