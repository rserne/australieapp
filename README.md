...
De vijftien prijzen staan in `dieren.js` (`PRIJZEN`), met per prijs de sleutel, de naam, de kleur van de kaart, de tekening op de munt (een sleutel uit `dieren-iconen.js` of een uit `PRIJS_ICONEN` in hetzelfde bestand: het pootje, het bestek, turfstreepjes, een ster, een zeven en de omtrek van Australië), de regel en de tekst. Regels: `eerste` (je eerste waarneming), `set` (al deze dieren, gezien of gegeten), `keuze` (n uit een lijst), `groep` (n soorten uit één groep), `soorten` (n soorten), `keer` (één dier n keer), `reeks` (n dagen achter elkaar iets gespot, op de kalenderdag van de waarneming), `tijd` (iets gespot tussen twee tijdstippen, over middernacht mag) en `regios` (iets gespot in n van de zeven streken van de groepsreis, via dezelfde indeling als de startpagina; de voorreis telt daar niet in mee). De datum op de kaart is die van de waarneming die de prijs compleet maakte. `check.js` controleert dat elk genoemd dier, elke groep en elke tekening bestaat en dat de regel de juiste velden heeft. De rekenregels zelf staan in `app.js` (`beoordeelPrijs`).

De vijftien: Eerste dier, Nachtwacht (iets gespot tussen 19.00 en 05.30 uur), Turver (15 soorten), Emoe! (vijf keer een emoe), Tassie (duivel, wombat en wallaby), Walvisseizoen (bultrug, dolfijn en zeehond), Papegaaien (drie van galah, kaketoe, regenbooglori en rosella), Koudbloedig (vier reptielen), Loopvogels (emoe en kasuaris), Bushtucker (kangoeroe, emoe en krokodil gegeten), Levend thuisgekomen (drie van tien gevaarlijke dieren), Zeven op een rij, Heel Australië (zes van de zeven streken), Grote lijst (30 soorten) en De grote vijf (kangoeroe, koala, wombat, echidna en vogelbekdier). Wie de broncode leest, kan spieken, maar daar kijkt onderweg niemand.
