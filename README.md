...
Wie `beheer = true` heeft, ziet in Praktisch onder het inlogblok de knop **Reizigers beheren**. Die opent een eigen scherm met de reizigerslijst: per persoon de delen van de reis, Gast en Beheer als chips (een tik zet er een aan of uit) en een kruisje om iemand uit de lijst te halen. De knop Reiziger toevoegen opent een schuifpaneel met naam, e-mailadres, wachtwoord, de delen en de chip Gast. Het wachtwoord maakt de app zelf: bij het openen staat er al een, drie dierennamen uit `dieren.js` en een getal, zoals `wombat-galah-koala-4821`. Het staat leesbaar in een gewoon tekstveld; de dobbelsteen ernaast maakt een nieuwe, de kopieerknop zet het op het klembord zodat je het meteen kunt doorsturen en toont dan even een vinkje, en zelf iets typen kan ook. Lukt kopiëren niet, dan staat het geselecteerd in het veld en zegt de statusregel dat. (Een melding onderin zou hier niet werken: het paneel ligt eroverheen.) Het is bewust geen wachtwoordveld: zodra iOS er een ziet, stelt Safari zelf een sterk wachtwoord voor en zet dat op hetzelfde moment in de sleutelhanger van de beheerder, onder het adres van de reiziger. Dat is vanuit een webpagina niet meer weg te halen, en bij het inloggen bood iOS daarna ook het account van de reiziger aan. Zonder wachtwoordveld herkent iOS het paneel niet als aanmeldformulier, dus ook het e-mailveld hoeft niet meer op `username` te staan en biedt iOS er niet het adres van de beheerder bij aan. Drie uit 45 woorden en een getal van vier cijfers is zo'n dertig bits aan willekeur: ruim genoeg tegen raden bij Nhost, dat herhaalde pogingen afremt. Een wachtwoord wijzigen of herstellen zit niet in de app: er is geen 'Wachtwoord vergeten', en de link uit een herstelmail van Nhost zou de app ook niet opvangen. Bewaar het wachtwoord dat je doorstuurt dus ook zelf. Na het toevoegen blijft het paneel open met een welkomstbericht voor de reiziger: het adres van de app, een lege regel, dan zijn e-mailadres en wachtwoord, hoe hij inlogt, wat er in de tabbladen zit (een gast krijgt geen regel over Notities, wie de voorreis of nareis doet een zin extra) en helemaal onderaan hoe je de app op het startscherm zet, voor iPhone en Android. De tekst is nog aan te passen, en gaat dan met Delen via het deelmenu van de telefoon (WhatsApp, Berichten, Mail) of met Kopiëren naar het klembord. De tekst zelf staat in `welkomTekst` in `app.js`. Het scherm gebruikt de foto van New South Wales (`reg-nsw.jpg`) als banner. De app maakt dan eerst het account aan via het gewone aanmeldpunt van Nhost Auth (`/signup/email-password`, met de naam als display name) en schrijft daarna de rij in `reizigers`. De beheerder blijft zelf ingelogd. Zo kan de lijst onderweg vanaf de telefoon worden bijgehouden; de Nhost-console is daar niet meer voor nodig. Jezelf verwijderen of je eigen beheer uitzetten kan niet in de app.
...
Raakt een reiziger zijn wachtwoord kwijt, dan is de enige uitweg het account in de Nhost-console weggooien en opnieuw aanmaken, en door de `on delete cascade` gaan zijn notities en waarnemingen dan mee. Uit de lijst halen ontneemt iemand de toegang, het account bij Nhost blijft bestaan.
...
Het tabblad Dieren (het pootje onderin, alleen voor wie is ingelogd, gasten ook) begint met Kans vandaag, de dieren uit het blok Dieren spotten van de dag, met de kans erbij. Daaronder staan alle 61 dieren uit `dieren.js` als lijst per groep, met een zoekveld en een chip per groep die op die groep filtert. Elke groep heeft een eigen kleur, met een variant voor het lichte en een voor het donkere thema. Het pootje volgt de kleur van de groep waarin het staat, en een dier dat de groep al zag krijgt die tint als achtergrond, met een gele teller erachter; is het ook gegeten, dan kleurt het bestekje ernaast oranje. Wie het zag of at, maakt voor de kleur niet uit. Met de chip Gespot houd je alleen de dieren over die de groep al heeft gezien, en met Gegeten alleen wat op het bord kwam. Beide chips verschijnen pas zodra er iets te filteren valt. Er staat er één tegelijk aan, en de twee tellingen blijven gescheiden. Zoeken en de chips werken samen, en er kan één groep tegelijk aanstaan. De aangezette groep schuift naar voren in de rij, vlak na Gespot en Gegeten: de rij scrolt zijwaarts, en een chip buiten beeld die de lijst leegfiltert, zie je anders niet. Blijft er niets over, dan noemt de melding de filters die aanstaan, met een knop Filters wissen. Gespot is geel, Gegeten oranje (dezelfde kleur als het bestekje), een aangezette groep heeft zijn eigen kleur en een aangevinkte naam krijgt een rand in de kleur van het filter dat aanstaat: geel bij Gespot, oranje bij Gegeten. Staat Gespot of Gegeten aan, dan verschijnt onder de chips een rij namen: Ik, dan de reizigers en daarna de gasten, elk op alfabet, van iedereen die iets heeft gespot of gegeten, met per naam het aantal soorten van dat filter. Elk getal in een chip houdt rekening met de andere filters, net als in Notities: 'Robbert 0' naast een aangezette groep Vogels betekent dat Robbert geen vogel at, en onder Gegeten tellen de groepschips gegeten soorten. Vink je er een of meer aan, dan is dat 'wij': alleen hun waarnemingen tellen nog mee, in de tellers per dier, de chips, Kans vandaag, de lijst onderaan en Tot nu toe, dat dan bijvoorbeeld zegt "Jij en Gemma hebben samen 12 dieren gespot". De keuze blijft op de telefoon staan (`aus_dwie`) tot je hem weghaalt of uitlogt; zet je het filter uit, dan telt weer de hele groep, en zet je het weer aan, dan staan dezelfde namen nog aangevinkt. Noteren gebeurt altijd onder je eigen naam, wat er ook aanstaat. Het zijn de dieren die je op deze route in het wild kunt tegenkomen. Wat je alleen op je bord ziet of vrijwel nooit in het wild, staat er bewust niet in. Eén tik op een dier is een waarneming, ook in Kans vandaag. Staat een dier uit de dagtekst niet in de lijst, dan krijgt het het pootje als icoon en noteert een tik het onder zijn eigen naam als ander dier. Alles wat zo is ingevoerd komt onderaan samen in de groep Overig, met het pootje als icoon en één regel per naam. Een tik daarop noteert hetzelfde dier nog een keer, dus wie na de eerste persoon dezelfde vogel ziet, hoeft de naam niet opnieuw te typen. Die groep staat er alleen als er iets in zit, en wordt gevuld uit de waarnemingen zelf, niet uit `dieren.js`. De app noteert het dier, wie het zag, de dag en de plaatselijke tijd, en de hele groep ziet het. Onder het raster staat de lijst Gespot: alle waarnemingen, nieuwste bovenaan, met een tussenkop per dag. Die lijst volgt dezelfde filters als het raster: de aangevinkte namen, Gespot of Gegeten (dan heet hij Gegeten) en de groep. Op de dagpagina staat bij elk dier uit het blok Dieren spotten hoe vaak de groep het al zag. Wie mis tikt, gebruikt Ongedaan maken in de melding. Een dier dat niet in het raster staat, gaat via de knop Iets anders gezien, met een naam erbij.

### Gespot en gegeten

Sommige dieren kun je in Australië ook op je bord krijgen, en dat is even leuk om bij te houden. Die dieren hebben in `dieren.js` de vlag `eet:true` (nu negen: kangoeroe, wallaby, emoe, zoutwaterkrokodil, barramundi, krab, koraalbaars, octopus en trevally). Haai staat er bewust niet bij: eten mag, maar wat als *flake* wordt verkocht is lang niet altijd een soort waarvan de stand het toelaat en krijgen in de lijst een bestekknopje naast de teller. Een tik op de regel noteert 'gezien', een tik op het bestek 'gegeten'. Zolang niemand het at, is het knopje leeg en gedempt; daarna kleurt het oranje met het aantal erin. Beschermde dieren als de koala, de wombat, de zeeschildpad en de dugong krijgen bewust geen knopje. Onder 'Tot nu toe' staat hoeveel er is gegeten en van hoeveel soorten de groep ze zowel in het wild zag als op het bord kreeg.
...
Bij de waarnemingen horen prijzen, zoals de badges van een stappenteller: een kaart in een eigen kleur met een ronde munt met een tekening erop, een korte tekst, de datum en een knop Delen. Ze gaan over je eigen waarnemingen (ook die in de wachtrij), niet over die van de groep en niet over de aangevinkte namen. En ze blijven verborgen tot je ze hebt: nergens staat een grijze medaille of "nog 2 soorten", zodat elke prijs een verrassing is. De eerste, G'day!, verdien je met je eerste tik; de kaart vertelt dan alleen hoeveel er nog te winnen zijn.

Verdien je met een tik een prijs, dan schuift de kaart omhoog zodra de waarneming is genoteerd; levert één tik er twee op, dan komen ze na elkaar. Daarna staat de medaille in Mijn kast op het scherm Stand (zie hieronder), de nieuwste voorop, en een tik erop opent de kaart opnieuw. Medailles tellen per reisdeel: wie de voorreis deed, heeft daar een eigen kast en begint op dag 1 net als de rest leeg. Delen geeft naam, aantal, tekst en datum als tekst aan het deelmenu van de telefoon. Haal je de waarneming weg die de prijs compleet maakte, dan verdwijnt de prijs weer uit de kast.

De zestien prijzen staan in `dieren.js` (`PRIJZEN`), met per prijs de sleutel, de naam, de kleur van de kaart, de tekening op de munt (een sleutel uit `dieren-iconen.js` of een uit `PRIJS_ICONEN` in hetzelfde bestand: het pootje, het bestek, turfstreepjes, een ster, een zeven en de omtrek van Australië), de regel en de tekst. Regels: `eerste` (je eerste waarneming), `set` (al deze dieren, gezien of gegeten), `keuze` (n uit een lijst), `groep` (n soorten uit één groep), `soorten` (n soorten), `keer` (één dier n keer), `reeks` (n dagen achter elkaar iets gespot, op de kalenderdag van de waarneming), `tijd` (iets gespot tussen twee tijdstippen, over middernacht mag) en `regios` (iets gespot in n van de zeven streken van de groepsreis, via dezelfde indeling als de startpagina; de voorreis telt daar niet in mee). De datum op de kaart is die van de waarneming die de prijs compleet maakte. `check.js` controleert dat elk genoemd dier, elke groep en elke tekening bestaat en dat de regel de juiste velden heeft. De rekenregels zelf staan in `app.js` (`beoordeelPrijs`).

De zestien: G'day! (je eerste waarneming), Nachtwacht (een avond- of nachtdier gespot tussen 19.00 en 05.30 uur), Streepjes zetten (15 soorten), Emoe! (een emoe), Tassie (duivel, wombat en wallaby), Kop boven water (twee van bultrug, dolfijn en zeehond), Herrie in de boom (drie van galah, kaketoe, regenbooglori en rosella), Zonnekloppers (vier reptielen), Loopclubje (emoe en kasuaris), Bushtucker (kangoeroe, emoe of krokodil gegeten), Gevaarlijk gezelschap (drie van tien beruchte dieren), Week zonder missers (zeven dagen op rij), Kriskras (zes van de zeven streken), Dertig op zak (30 soorten), Geluksvogel (boomkangoeroe of quoll) en De grote vijf (kangoeroe, koala, wombat, echidna en vogelbekdier). Wie de broncode leest, kan spieken, maar daar kijkt onderweg niemand.

### Reisdelen en het scherm Stand

Sinds uitgave 220 rekent Dieren per **reisdeel**: de voorreis (dag onder 0), de groepsreis (dag 0 tot en met 29) en de nareis (dag boven 29). Het reisdeel volgt uit het dagnummer dat al bij elke waarneming staat; aan Nhost verandert niets. Het tabblad Dieren telt alleen het reisdeel van vandaag: de chip Gespot, de groepstellers, de gekleurde dierregels, de namenrij en de lijst Gespot. Zo begint iedereen op dag 1 bij nul en neemt wie de voorreis deed geen voorsprong mee. Wat er eerder is gespot, blijft bewaard en staat in Stand. Gasten tellen op Dieren gewoon mee: zij horen bij het gezelschap. De lijst Gespot toont de laatste tien waarnemingen binnen de filters die aanstaan, met een knop *Alles tonen* voor de rest; het tabblad opnieuw openen begint weer bij tien. De chip Gespot en de namenrij tellen soorten, de lijst toont waarnemingen. De app haalt wel altijd alle waarnemingen op, want de tellers, de stand en de medailles rekenen met de volledige lijst.

**Stand** is het tweede scherm van het tabblad Dieren, achter de beker rechtsboven in de kop; `‹ Dieren` in de kop brengt je terug, en wie het tabblad opnieuw opent, begint weer op het noteerscherm. Stand is puur kijken: noteren en weghalen blijven op Dieren. Van boven naar beneden:

- **Mijn kast**: mijn aantal dieren, soorten (met het totaal aantal soorten in de app ernaast) en medailles, daaronder mijn medailles. Doe ik aan meer dan één reisdeel mee (volgens de reizigerslijst, of omdat ik ergens toch iets noteerde), dan staan er bovenin chips om van reisdeel te wisselen; die keuze stuurt het hele scherm. Standaard staat het reisdeel van vandaag.
- **Stand**: een podium met de eerste drie, nummer 1 in het midden, met de naam en het aantal dieren erboven en het aantal soorten groot in het blok. Reisgenoten en gasten doen allebei mee. Sta ik er niet op, dan volgt mijn eigen trede eronder met mijn plek. Ben ik in dit reisdeel de enige met waarnemingen, dan is er geen blok Stand.
- **Mijn waarnemingen**: alleen de mijne in het gekozen reisdeel, nieuwste bovenaan, met een tussenkop per dag; na tien een knop *Alles tonen*.

De rekenregel van de stand: rangschikken op het aantal **soorten**, bij gelijke stand op het aantal dieren, en daarna op wie het eerst zo ver was. Alleen *gezien* telt, *gegeten* niet. *Iets anders gezien* telt per verschillende naam (hoofdletters genegeerd). Medailles tellen niet mee: die komen grotendeels uit dezelfde waarnemingen voort en zijn de beloning, niet de score.
